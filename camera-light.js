    ...
    function CameraLightNode(config) {
        ...
        async function setLight(session, mode, brightness = 100) {
            try {
                ...
                // Применяем настройки ко всем профилям (General, Day, Night)
                for (let profileIndex = 0; profileIndex < table[0].length; profileIndex++) {
                    for (let subIndex = 0; subIndex < table[0][profileIndex].length; subIndex++) {
                        table[0][profileIndex][subIndex].Mode = mode;
                        table[0][profileIndex][subIndex].PercentOfMaxBrightness = brightness;
                        
                        if (mode === "Manual") {
                            table[0][profileIndex][subIndex].MiddleLight[0].Light = brightness;
                        }
                    }
                }
